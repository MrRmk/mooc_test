<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      ...
      <el-form-item label="后期制作Id" prop="postPersonnel">
        <el-input
          v-model="queryParams.postPersonnel"
          placeholder="请输入后期制作Id"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="后期制作Id" prop="fileUrl">
        <el-input
        ...
        <el-form-item label="状态(0未审核，1审核不通过,2审核通过)" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态(0未审核，1审核不通过,2审核通过)" clearable size="small">
          <el-option
        ...
        <el-select v-model="queryParams.missionId" placeholder="请选择任务Id" clearable size="small">
          <el-option label="请选择字典生成" value="" />
        </el-select>
      </el-form-item>
        <el-form-item label="是否删除NY" prop="isDeleted">
        <el-select v-model="queryParams.isDeleted" placeholder="请选择是否删除NY" clearable size="small">
          <el-option
    ...
    <el-table v-loading="loading" :data="knowledgeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="后期制作Id" align="center" prop="postPersonnel" />
      <el-table-column label="后期制作Id" align="center" prop="fileUrl" />
      <el-table-column label="状态(0未审核，1审核不通过,2审核通过)" align="center" prop="status" :formatter="statusFormat" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="后期制作Id" prop="postPersonnel">
          <el-input v-model="form.postPersonnel" placeholder="请输入后期制作Id" />
        </el-form-item>
        <el-form-item label="后期制作Id" prop="fileUrl">
          <el-input v-model="form.fileUrl" placeholder="请输入后期制作Id" />
        </el-form-item>
        <el-form-item label="状态(0未审核，1审核不通过,2审核通过)">
        ...
        <el-form-item label="任务Id">
          <el-select v-model="form.missionId" placeholder="请选择任务Id">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
        <el-form-item label="是否删除NY">
          <el-select v-model="form.isDeleted" placeholder="请选择是否删除NY">
            <el-option
              v-for="dict in isDeletedOptions"
              :key="dict.dictValue"
              :label="dict.dictLabel"
...
<script>
import { listKnowledge, getKnowledge, delKnowledge, addKnowledge, updateKnowledge, exportKnowledge } from "@/api/system/knowledge";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除知识点功能编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有知识点功能数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
